...
```bash
git clone https://github.com/bnam91/module_auth ~/Documents/github_cloud/module_auth
# 이미 있으면:
git -C ~/Documents/github_cloud/module_auth pull
```

---

## 3. API Key 모듈 설치 (module_api_key)

Notion, 기타 외부 API 키를 관리하는 모듈이야.

1. 아래 Google Drive 폴더에서 **폴더 전체 및 `.env` 파일** 다운로드:
   ```
   https://drive.google.com/drive/u/0/folders/1opkiTG09SP9_a3pHUy41DcnCVjUOPPK0
   ```
2. 아래 경로에 배치:
   ```
   ~/Documents/github_cloud/module_api_key/
   ```
